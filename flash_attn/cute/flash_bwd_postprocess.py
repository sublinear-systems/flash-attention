    ...
    def _get_tiled_mma(self):
        if const_expr(self.arch // 10 in [8, 12]):
            ...
            tiled_mma = sm90_utils_basic.make_trivial_tiled_mma(
                ...
                + (1,),
                tiler_mn=(64, tiler_mn_dQ[1] if not self.dQ_swapAB else tiler_mn_dQ[0]),  # pin WGMMA atom M=64; M (tile_m/tile_hdim) is looped downstream by partition_shape_C. fixes hd192 dKV postprocess (M-mode crash)
            )
        else:
